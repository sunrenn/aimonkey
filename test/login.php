...
/*
    ...
    可能的测试场景：
        ...
        - 同一个人（邮箱）相同的用户名注册，应该被提示错误
        - 同一个人（邮箱）正常注册（用户名不同），超过系统设置的限额（默认3），应该被提示错误，但现在的情况是可以无限成功注册，超过数量先之后知识头像的状态编程锁定，而且还能正常登录！这是谁干的？不是腾讯就是阿里（阿里并没参与这次行动）。这虽然完全违背我的初衷，而且相当难以理解，不过做已经做了就先这么着吧，也挺有意思的。除了会多占用微不足道的空间，也没什么大不了的。但如果真的有不止一打用户这么干了，怎么处理？毕竟没有给出明确的提示。所以这还是一个莫名其妙的“坑”，可见注册协议必要性，或者干脆改成超过限额就直接拒绝注册。
